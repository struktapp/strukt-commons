...
class Str extends ValueObject{
	...
	public function prepend(string $str):static{
		...
		return new Str(sprintf("%s%s", $str, $this->value));
	}
	...
	public function concat(string $str):static{
		...
		return new Str(sprintf("%s%s", $this->value, $str));
	}
	...
	*/
	public function toSnake():static{
		...
		$str = new Str(preg_replace($pattern, "_", $this->value));
	...
	*/
	...
	public function replace(array|string $search, array|string $replace):static{
		...
		return new Str(str_replace($search, $replace, $this->value));
	}
	...
	*/
	public function replaceAt(array|string $replace, array|string $start, ?int $length = null):static{
		...
		if(is_null($length))
			return new Str(substr_replace($this->value, $replace, $start));

		return new Str(substr_replace($this->value, $replace, $start, $length));
	}
	...
	*/
	public function replaceFirst(array|string $search, array|string $replace):static{
    	...
    	return new Str(preg_replace("/".$search."/", $replace, $this->value, 1));
	}
	...
	*/
	public function part(int $start, int $end):static{
		...
		return new Str(substr($this->value, $start, $end));
	}
	...
	public function empty():bool{
		...
		return strlen($this->value) === substr_count($this->value, ' ') ? true : false;
	}
